(function () {
  ...
  function getConfigFromQuery() {
    ...
    var trackingScript = Array.prototype.find.call(scripts, function (s) {
      return s.src && s.src.indexOf('tracking-script16') !== -1;
    });

    if (!trackingScript || trackingScript.src.indexOf('?') === -1) {
      console.log('[Tracking] Script not found or query params missing.');
      return {};
    }

    var queryString = trackingScript.src.split('?')[1];
    ...
    return {
      ...
      ctaTexts: (params.get('ctaText') || '').split(',').map(normalize)
    };
  }

  var CONFIG = getConfigFromQuery();
  ...
  var scrollTracked = { '20': false, '50': false };
  ...
  function getPayload(eventName, clickedText) {
    ...
    if (eventName === 'any_cta') {
      return { page: page, cta_clicked: clickedText.slice(0, 100) };
    }
    return { page: page };
  }

  function sendToAllPlatforms(eventName, clickedText) {
    var payload = getPayload(eventName, clickedText || '');
    ...
    if (
      typeof lintrk === 'function' &&
      CONFIG.linkedinEventId &&
      (eventName === 'any_click' || eventName === 'any_cta')
    ) {
  ...
  function handleClick(event) {
    ...
    var normalizedClicked = normalize(clickedText);

    sendToAllPlatforms('any_click');

    if (CONFIG.ctaTexts.indexOf(normalizedClicked) !== -1) {
